...
int main(int argc, char **argv)
{
  ...
  if (nparams == 0)
    ...
    mask_x = calloc(nchains * nwavr * nelements, sizeof(unsigned short));
    mask_y = calloc(nchains * nwavr * nelements, sizeof(unsigned short));
    if (mask_filename[0] != 0)
      {
        ...
        printf("\nMask image -- Opening file : %s\n", mask_filename);
            ...
            // Check flux normalization DONT NEED FOR MASK
            //for (w = 0; w < nwavi; ++w)
            //  {
              //  ftot = 0.0;
              //  for (j = 0; j < in_naxes[0]; ++j)
            ...
            for (w = 0; w < nwavi; ++w)
              {
                /* 1) Dummy int counts through the elements
                *  2) i,j count through the pixels
                *  3) dtemp counts up flux in the input image. All are 1 or 0.
                *  Once we get to 1 element, we add this in */
                dummy_int = 0;
                if (offset < 0)
                  i = -offset;
                ...
                while (dummy_int < nelements)
                  {
                    if (dtemp >= 1)
                      {
                        dtemp -= 1.0; /* We've used up one unit of flux */
                        mask_x[w * nelements + dummy_int] = i + offset;
                        mask_y[w * nelements + dummy_int] = j + offset;
                        dummy_int++;
                      }
                    else
                      {
                        ...
                        dtemp += mask_image[w * in_naxes[0] * in_naxes[0] + i + j * in_naxes[0]] * nelements; /* Add the flux from the next pixel...*/
                      }
                  }
              }
            ...
            if (nwavi < nwavr)
              {
                ...
                for (w = nwavi; w < nwavr; ++w)
                  {
                    memcpy(&mask_x[w * nelements], &mask_x[0], nelements * sizeof(unsigned short));
                    memcpy(&mask_y[w * nelements], &mask_y[0], nelements * sizeof(unsigned short));
                  }
              }
            ...
            for (j = 0; j < nchains; ++j)
              {
                memcpy(&mask_x[j * nwavr * nelements], &mask_x[0], nwavr * nelements * sizeof(unsigned short));
                memcpy(&mask_y[j * nwavr * nelements], &mask_y[0], nwavr * nelements * sizeof(unsigned short));
              }

            free(in_naxes);
            free(mask_image);
            printf("Mask image -- Digitized into nelements = %ld\n", nelements);
        ...
        if ((nparams == 0) || (current_elt < nelements)) /* Attempt image movement rather than parametric model movement */
          {
                maskflag = TRUE;
                zerostep = TRUE;
                //while (mask_x[new_x]==0 && mask_y[new_y] ==0 )
                //  {
                  //    zerostep = TRUE;
                // select step
                while (maskflag == TRUE)
                    ...
                    switch (steptype)
                      {
                        ...
                        case STEP_MEDIUM: /* Up to 2 steps in x and y direction */
                          xstep = (rlong % 5) - 2;
                          rlong /= 5;while (mask_x[new_x]==0 && mask_y[new_y] ==0 )
                          ystep = (rlong % 5) - 2;
                          rlong /= 5;
                        break;
                ...
                if (mask_element_x[new_x]!=0 && mask_element_y[new_y] !=0 )
                //if (image_mask[new_pos] !=0 )
                {
                  maskflag=FALSE;
                  printf("YES");
                }
...
void initialize_mask(int iChain, double *image_mask, unsigned short *mask_element_x, unsigned short *mask_element_y, unsigned short *mask_x, unsigned short *mask_y,
                      unsigned short axis_len, int nwavr, long nelements, char *mask_filename)
{
  ...
  memcpy(&mask_element_x[0], &mask_x[iChain * nwavr * nelements], nwavr * nelements * sizeof(unsigned short));
  memcpy(&mask_element_y[0], &mask_y[iChain * nwavr * nelements], nwavr * nelements * sizeof(unsigned short));

  for (w = 0; w < nwavr; ++w)
    for (i = 0; i < nelements; ++i)
      image_mask[w * axis_len * axis_len + mask_element_y[w * nelements + i] * axis_len + mask_element_x[w * nelements + i]]++;
}
